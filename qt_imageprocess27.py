...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr  8 10:14:49 2018
...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 30 15:42:37 2018
...
#            print ("222")
    ...
    def SaltAndPepper(self,img_temp,percentage=0.2):
        NoiseImg=img_temp
        NoiseNum=int(percentage*img_temp.shape[0]*img_temp.shape[1])
        for i in range(NoiseNum):
            randX=random.randint(0,img_temp.shape[0]-1)
            randY=random.randint(0,img_temp.shape[1]-1)
...
#        for i in range(self.allFiles.count()):
        for i in range(self.allFiles.count()):
            ...
            temp7="QTGUITarget8/"+self.allFiles.item(i).text()
            temp8="QTGUITarget9/"+self.allFiles.item(i).text()
            temp9="QTGUITarget10/"+self.allFiles.item(i).text()
            temp10="QTGUITarget11/"+self.allFiles.item(i).text()
            temp2=self.allFiles.item(i).text()
            self.allFiles2.addItem(temp2)
            print (temp)
            print (temp1)
            self.img_processed=cv.imread("QTGUI_Label/template.bmp",0)
...
#                    cv.imwrite(temp3,img_target_temp)
                    ...
                    img_Template=cv.imread("QTGUI_Label/template.bmp",0)
                    h,w=img_Template.shape[:2]
                    img_Ori=cv.imread(temp5,0)
                    img_Ori2=cv.imread(temp5,0)
                    methods=['cv.TM_CCOEFF','cv.TM_CCOEFF_NORMED','cv.TM_CCORR','cv.TM_CCORR_NORMED','cv.TM_SQDIFF','cv.TM_SQDIFF_NORMED']
                    meth=methods[0]
                    method=eval(meth)
                    res=cv.matchTemplate(img_Ori,img_Template,method)
                    print (res.shape)
                    ...
                    print (top_left)
                    roi_size=30
                    img_target_temp3=copy.copy(img_Ori2[top_left[1]-roi_size:top_left[1]+h+roi_size,top_left[0]-roi_size:top_left[0]+w+roi_size])
                    cv.imwrite(temp6,img_target_temp3)
                    print (img_target_temp3.shape)
#                    rotation transform
#                    for angle in np.arange(-2,2.1,0.2):
##                        temp_result=temp8.split('/')
##                        print (temp_result)
#                        temp_result=temp8.split('.')[0]+'_'+str(angle)+'.bmp'
#                        (H)=cv.getRotationMatrix2D(center=(img_target_temp3.shape[1]/2,img_target_temp3.shape[0]/2),angle=angle,scale=1)
#                        img_target_temp4=cv.warpAffine(img_target_temp3,H,(img_target_temp3.shape[1],img_target_temp3.shape[0]))
#                        img_target_temp5=copy.copy(img_target_temp4[roi_size:img_target_temp4.shape[0]-roi_size,roi_size:img_target_temp4.shape[1]-roi_size])
#                        cv.imwrite(temp_result,img_target_temp5)
#                    target_size transform
#                    for size_change in np.arange(-5,5.1,1):
##                        temp_result=temp8.split('/')
##                        print (temp_result)
#                        size_change=int (size_change)
#                        print (size_change)
#                        temp_result=temp9.split('.')[0]+'_'+str(size_change)+'.bmp'    
#                        img_target_temp5=copy.copy(img_target_temp3[roi_size+size_change:img_target_temp3.shape[0]-roi_size-size_change,roi_size+size_change:img_target_temp3.shape[1]-roi_size-size_change])
#                        img_target_temp6=cv.resize(img_target_temp5,(w,h))
#                        cv.imwrite(temp_result,img_target_temp6)
                        
                    img_target_temp5=copy.copy(img_target_temp3[roi_size:img_target_temp3.shape[0]-roi_size,roi_size:img_target_temp3.shape[1]-roi_size])
                    img_target_temp6=self.SaltAndPepper(img_temp=img_target_temp5,percentage=0.2)
                    cv.imwrite(temp10,img_target_temp6)
                    
#                    print (img_target_temp5.shape)
...
#                    size_change=2
##                    img_target_temp6=copy.copy(img_target_temp3[roi_size+size_change:img_target`_temp3.shape[0]-roi_size-size_change,roi_size+size_change:img_target_temp3.shape[1]-roi_size-size_change])
#                    img_target_temp6=copy.copy(img_target_temp3[roi_size:img_target_temp3.shape[0]-roi_size,roi_size:img_target_temp3.shape[1]-roi_size])
#                    print (img_target_temp6.shape)
##                    img_target_temp6.resize(h,w)
##                    print (img_target_temp6.shape)
#                    cv.imwrite(temp9,img_target_temp6)
